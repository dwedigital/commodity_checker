# The original single-shot commodity suggester: it searches the tariff API with
# the whole description and asks Claude to pick or invent a code in one call.
# Kept so the eval harness can compare the retrieve-then-walk pipeline against
# the old behaviour (TARIFFIK_PIPELINE=legacy). LlmCommoditySuggester delegates
# here in that mode. See claude/implementations/hierarchical-classification.md.
class LegacyCommoditySuggester
